...
''' 
''' Copied from http://www.codeproject.com/KB/IP/FtpClient.aspx and adapted for Sirius use (including some refactoring 
''' to follow Sirius coding guidelines)
''' </remarks>
...
#Region "Directory functions"
    ...
    Public Function ListDirectory(Optional ByVal directory As String = "") As List(Of String)
        ...
        Dim response As String = GetStringResponse(ftp)
        'replace CRLF to CR, remove last instance
        response = response.Replace(ControlChars.CrLf, ControlChars.Cr).TrimEnd(ControlChars.Cr)
        'split the string into a list
        Dim result As New List(Of String)
        result.AddRange(response.Split(ControlChars.Cr))
        Return result
    End Function
    ...
    Public Function ListDirectoryDetail(Optional ByVal directory As String = "") As FTPdirectory
        ...
        response = response.Replace(ControlChars.CrLf, ControlChars.Cr).TrimEnd(ControlChars.Cr)
        'split the string into a list
        Return New FTPdirectory(response, _lastDirectory)
    End Function
#End Region
...
#End Region
    ...
    Sub New(ByVal line As String, ByVal path As String)
        ...
        Else
            ...
            Try
                ' JG 21/11/2011: Format appears to be mm-dd-yy hh-mmXX regardless of Locale etc.
                ' _fileDateTime = Date.Parse(m.Groups("timestamp").Value)
                Dim d = m.Groups("timestamp").Value
                _fileDateTime = New DateTime(2000 + CInt(d.Substring(6, 2)), CInt(d.Substring(0, 2)), _
                                             CInt(d.Substring(3, 2)), CInt(d.Substring(10, 2)), CInt(d.Substring(13, 2)), 0)
                If d.Substring(15, 2).ToUpper = "PM" Then _fileDateTime = _fileDateTime.AddHours(12)
            Catch ex As Exception
...
Public Class FTPdirectory
    ...
    Public Function DirectoryExists(ByVal directoryname As String) As Boolean
        For Each ftpfile As FTPfileInfo In Me.GetDirectories
            If ftpfile.Filename = directoryname Then
                Return True
            End If
        Next
        Return False
